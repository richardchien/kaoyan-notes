...
| 名称 | 时间 | 空间 | 稳定 | 特点 |
| --- | --- | --- | --- | --- |
| 冒泡排序 | $O(n^2)$ | $O(1)$ | ✔️ | 实现简单，最坏时需要 $O(n^2)$ 次交换。 |
| 插入排序 | $O(n^2)$ | $O(1)$ | ✔️ | 相比选择排序，优点在于找到插入点之后即可停止，对于大致有序的序列来说较快。<br><br>缺点在于对数组来说，每次插入需要移动较多元素（$O(n^2)$ 次交换），但并没有改变渐近复杂度。 |
...
| 归并排序 | $O(n \log n)$ | $O(n)$ | ✔️ | 快且稳定，缺点是不能原地。 |
| 堆排序 | $O(n \log n)$ | $O(1)$ | ❌ | 原地堆排序只能用于数组。 |
| 快速排序 | $O(n \log n)$ | $O(1)$ | ❌ | 相比堆排序，平均情况下常系数更小，且可用于链表，缺点是若选择的轴点过小或过大，复杂度会升高为 $O(n^2)$。 |
| 桶排序 | $O(n + m)$ | $O(m)$ | ✔️ | 速度快，适合数据量大但数据范围小的情况（m << n），若数据范围较大则很浪费空间甚至无法实现。 |
| 基数排序 | $O(k \cdot (n + m))$ | $O(m)$ | ✔️ | 基于桶排序，依赖其稳定性，适用于多字段元素的排序，例如元组、字符串等，同时，每个字段的数据范围应当较小。 |
