...
`protcseg` 中对各数据段寄存器进行了设置，这时已完成进入保护模式的所有工作。最后设置了 EBP 和 ESP，分别为栈帧基地址（这里为 0x0）和栈底地址（这里为 0x7C00，栈是向内存低地址增长的，也就是说 PUSH 时 ESP 会减小，由于逻辑上的栈底的内存地址最高，所以有些地方也称为「栈顶」），然后调用了 C 语言编写的 `bootmain` 函数，进入加载内核的过程。
